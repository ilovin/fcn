...
from lib.utils.upsampling import bilinear_upsample_weights
...
class Network(object):
    ...
    @layer
    def upsample(self, input, c_o,stride = 2, name = 'upsample'):
        upsample_filter_np = bilinear_upsample_weights(stride,c_o)
        upsample_filter_tensor = tf.constant(upsample_filter_np)

        downsampled_logits_shape = tf.shape(input)
        upsampled_logits_shape = tf.stack([
            downsampled_logits_shape[0],
            downsampled_logits_shape[1] * stride,
            downsampled_logits_shape[2] * stride,
            downsampled_logits_shape[3]
        ])
        with tf.variable_scope(name) as scope:
            upsampled_logits = tf.nn.conv2d_transpose(input,
    ...
    @layer
    def getValidfeed(self, input, n_classes, name):
        with tf.name_scope(name=name) as scope:
            class_labels = [i for i in  range(n_classes)]
            class_labels.append(255)#not care label is 255
            # class_labels = np.array(class_labels,dtype=np.float32)
            upsampled_logits_batch,annotation_batch = input[0],input[1]
            annotation_batch = tf.cast(tf.image.resize_images(annotation_batch, tf.constant([96,96])),dtype=tf.uint8)

            labels = get_valid_logits_and_labels(annotation_batch_tensor=annotation_batch,
                                                 logits_batch_tensor=upsampled_logits_batch, class_labels=class_labels)
            valid_labels_batch_tensor, valid_logits_batch_tensor = labels
            return valid_logits_batch_tensor,valid_labels_batch_tensor
